...
class CreateIdeaPage extends Component {
	...
	onSubmit = async (e) => {
		...
		if (!this.state.photos.length)
			return this.setState({ error: 'Idea must have a photo' })
		if (!this.state.type.length)
			return this.setState({ error: 'Idea must be either a good or service' })
		...
		if (this.state.currentLocation) {
			...
			if (!this.state.address.length)
				return this.setState({ error: 'Idea must have an address' })
			else
	...
	render() {
		return (
			<form onSubmit={this.onSubmit} encType="multipart/form-data" className="createIdea">
				...
				<div className="postInformation">
					<label><strong>Idea Type:</strong></label>
					<label>Good</label>
					...
					<br/>
					<input type="submit" className="createAccountIdea" value="Create Idea"/>
				</div>
			</form>
		)
	}
